...
class ParquetCombinerRDDIntegrationTest extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("Full pipeline execution with command line arguments") {
    ...
    assert(Set("apple", "banana").contains(nyTopItem(0).getAs[String]("item_name")), 
    ...
    rankingCheck.foreach { row =>
      ...
      assert(maxRank === uniqueRanks.toString, s"$location should have maximum rank equal to the number of items")
    }
  }
  ...
  test("Full pipeline execution with alternate topX value") {
    ...
    assert(rankingCheck(0).getAs[String]("item_rank") === "1", "Should have rank 1")
    assert(rankingCheck(1).getAs[String]("item_rank") === "2", "Should have rank 2")
  }
  ...
  test("Duplicate detection_oid handling") {
    ...
    assert(results.length === 2, "Should have 2 distinct items after deduplication")
    ...
    assert(Set("apple", "banana").contains(topItemName), 
